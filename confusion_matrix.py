...
def display(confusion_matrix, categories, output_path):
    ...
    for i in range(len(categories)):
        ...
        precision = float(confusion_matrix[id, id] / total_predicted) * 100
        recall = float(confusion_matrix[id, id] / total_target) * 100
        #f1score = float(2 * ((precision * recall)/(precision + recall)))
        #print(total_predicted, total_target)
        
        #print('precision_{}@{}IOU: {:.2f}'.format(name, IOU_THRESHOLD, precision))
        #print('recall_{}@{}IOU: {:.2f}'.format(name, IOU_THRESHOLD, recall))
        
        #results.append({'category' : name, 'precision_@{}IOU'.format(IOU_THRESHOLD) : precision, 'recall_@{}IOU'.format(IOU_THRESHOLD) : recall, 'f1score_@{}IOU'.format(IOU_THRESHOLD) : f1score})
